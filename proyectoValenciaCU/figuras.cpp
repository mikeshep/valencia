...
void CFiguras::renderSphere( float cx, float cy, float cz, float r, int p, GLuint text )
{
...
void CFiguras::esfera(GLfloat radio, int meridianos, int paralelos, GLuint text )
{
	...
	/*for(double p =-3.14/2; p < 3.14/2; p += angulop)
	{
		for(double m = 0; m < 3.14*2; m+=angulom)
		{
			...
			glTexCoord2f(ctext_s*m, -ctext_t*(p+angulop));
			...
			glTexCoord2f(ctext_s*(m+angulom), -ctext_t*(p+angulop));
			...
			glTexCoord2f(ctext_s*(m+angulom), -ctext_t*p);
			...
			glTexCoord2f(ctext_s*m, -ctext_t*p);
